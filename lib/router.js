...
Router.prototype.getItems = function(request, reply) {
    ...
    var sources = Array.isArray(request.query.sources) ? request.query.sources : [request.query.sources];
    var types   = Array.isArray(request.query.types) ? request.query.types : [request.query.types];
    ...
    return Database.getQuery(search, types, sources, function(err, query, exist) {
        ...
        if (exist) {
            //Cache.put('queries.' + '.' + query._hash, query, 60000);
        }

        return Database.getItems(first, last, query.query, function(err, resObj) {
            if (err) {
                return reply({'status' : 'not_found', 'error' : err}).code(400);
            }
