 ...
 *  Get item
 */
Database.prototype.getQuery = function(search, types, sources, done) {
    if (!types || !sources) {
        return done(new Error('Invalid query'));
    }

    var qHash = Utils.generateQueryHash(search, types, sources);

    if (Cache.get('queries.' + '.' + qHash)) {
        console.log('returning Cache for getQuery : ' + query._hash);
        return done(null, Cache.get('queries.' + '.' + qHash));
    };
    ...
    return Models.model['query'].findOne({_hash : qHash}, function(err, doc) {
        ...
        var newQuery = {
            _hash : qHash,
            type : 'standard',
            query : Utils.generateQuery(search, types, sources),
            created : new Date(),
...
Database.prototype.getItems = function(first, last, query, done) {
    ...
    if (!first && !last) {
        query['_sort'] = {$lte : +(new Date())};
    }

    if (first && !last) {
        query['_sort'] = {$gt : first};
    }
    ...
    if (first && last) {
        var tempF = JSON.parse(JSON.stringify(query));
        tempF['_sort'] = {$lt : last};
        var tempL = JSON.parse(JSON.stringify(query));
        tempL['_sort'] = {$gt : first};
    ...
    console.time('items');
    return Models.model['item'].find(query).count(function(err, count) {
        return Models.model['item'].find(query).sort({_sort : -1}).limit(50).exec(function(err, docs) {
