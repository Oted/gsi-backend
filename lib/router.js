...
Router.prototype.postItem = function(request, reply) {
    ...
    raw.token = request.yar.get('token');
...
Router.prototype.getInfo = function(request, reply) {
    ...
    Database.addOrUpdateUserState(state, ip, token, function(err) {
        if (err) {
    ...
    return Database.getInfo(function(err, doc) {
        ...
        return reply(doc).code(200).state('token', token);
    });
};
