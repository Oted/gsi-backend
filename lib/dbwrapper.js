...
//enum schema types
var itemTypes       = ['youtube', 'img', 'gif', 'gifv', 'soundcloud', 'vimeo', 'vine', 'text', 'video', 'twitch', 'ted', 'instagram', 'sound', 'other'];
 ...
 */
DbWrapper.prototype.vote = function(hash, ip, value, adjective, done) {
    if (!hash) {
 ...
 */
DbWrapper.prototype.getBestRatings = function(amount, done) {
    amount = amount > 25 ? 25 : amount;

    var query = ItemModel.find({enabled : true}, {_id : 0, _hash : 1}).sort({score : -1}).limit(amount);
    query.exec(done);
}; 
