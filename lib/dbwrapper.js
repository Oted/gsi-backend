...
//enum schema types
var itemTypes       = ['youtube', 'img', 'gif', 'soundcloud', 'vimeo', 'text', 'video', 'instagram', 'sound', 'other'];
var addjectiveTypes = ['arty', 'funny', 'inspiring', 'creepy', 'beautiful', 'cute', 'crazy', 'offensive', 'boring', 'uggly', 'disturbing'];
...
function DbWrapper(url, done) {
    ...
    db.once('open', function() {
        ...
        var ratingSchema = new Mongoose.Schema({
            ...
            addjective  : { type: String, enum: addjectiveTypes },
...
DbWrapper.prototype.getItems = function(amount, first, last, done) {
    ...
    if (first && !last) {
        query = ItemModel.find({enabled : true, _sort : {$lt : first}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }

    //last is the sort time of the latest item seen
    if (!first && last) {
        query = ItemModel.find({enabled : true, _sort : {$gt : last}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }

    //outside of the seen window
    if (first && last) {
        query = ItemModel.find({enabled : true, _sort : {$gt : last, $lt : first}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }
...
DbWrapper.prototype.vote = function(hash, ip, value, addjective, done) {
    ...
    if (addjective && addjectiveTypes.indexOf(addjective) === -1) {
...
DbWrapper.prototype.getRatings = function(hash, done) {
    RatingModel.find({_hash  : hash}, {_id : 0, ip : 0, __v : 0, addjective : 0}, done);
}; 
...
internals.dealWithHotItems = function(item) {
    ...
    // console.log('New hotItem list : ');
    // console.log(JSON.stringify(hotItems, null, " "));
    return hotItems;
};
