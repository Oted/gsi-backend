...
function Router(aServer, aDbWrapper) {
    ...
    //route for addjectives
    server.route({
        method: 'GET',
        path: '/api/addjectives',
        handler: that.getAddjectives
    });
};
...
Router.prototype.getItems = function(request, reply) {
    ...
    dbWrapper.getItems(amount, first, last, function(err, doc) {
        if (err) {
...
Router.prototype.noVote = function(request, reply) {
    ...
    var addjective  = request.query.addjective.toLowerCase();

    if (!hash) {
        return reply({'err' : 'not_found'}).code(400);
    }
    
    dbWrapper.vote(hash, ip, 0, addjective, function(err, doc) {
        if (err) {
            console.error(err);
...
Router.prototype.upVote = function(request, reply) {
    var ip          = request.info.remoteAddress;
    var hash        = encodeURIComponent(request.params.hash);
    var addjective  = request.query.addjective;

    if (!hash) {
        return reply({'err' : 'not_found'}).code(400);
    }
    
    dbWrapper.vote(hash, ip, 1, addjective, function(err, doc) {
        if (err) {
            console.error(err);
            return reply().code(304);
        }
 ...
 *  Last    the latest date of all the items already fetched
 */
Router.prototype.getAddjectives = function(request, reply) {
    var ip          = request.info.remoteAddress;
    var amount      = request.query.amount || 3;
    
    console.log(ip + ' requested ' + amount + ' addjectives!'); 
    dbWrapper.getAddjectives(amount, function(err, doc) {
        if (err) {
            return reply({'status' : 'not_found', 'error' : err}).code(400);
        }
