...
const BANNED_FRAGMENTS = ['ignentertainment', 'ign', 'don'];
...
/**
 * returns an array with the most related things;
 */
module.exports.getRelatedFragments = function(docs, fragment) {
    var map = {}, all = [];

    docs.hits.hits.forEach(item => {
        item._source.fragments.forEach(f => {
            if (f.indexOf(fragment) > -1 || fragment.indexOf(f) > -1 || BANNED_FRAGMENTS.indexOf(f) > -1 || parseInt(f)) {
                return;
            }
    ...
    all = all.sort(function(f1, f2) {
        return map[f2] - map[f1];
    }).map(function(f) {
        return {
            'fragment' : f,
            'score' : map[f]
...
module.exports.decideSourceType = function(raw) {
    ...
    var ifunnyRex   = new RegExp(/ifunny\.co/gi);
...
module.exports.convertData = function(raw, done) {
    ...
    if (raw.title && raw.title.indexOf('commentshare') > 1) {
        raw.title = raw.title.split('youtu').shift();
    }
