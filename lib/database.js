...
Database.prototype.getFragments = function(amount, type, done) {
    ...
    switch (type) {
        ...
        case "random" :
            return Models.model['title-fragment'].find({}).exec(function(err, docs) {
                if (err) {
                    return done(err);
                }
...
Database.prototype.getItems = function(requestQuery, query, done) {
    if (Cache.get('items.' + requestQuery.first + '.' + requestQuery.last + '.' + query._hash)) {
        console.log('returning Cache for getItems : items', query._hash);
...
Database.prototype.createItem = function(raw, done) {
    ...
    if (!raw.hash) {
        raw._hash = Utils.generateHash(raw);
    }
...
Database.prototype.addOrUpdateUserState = function(state, ip, token, done) {
    ...
    return Models.model['user'].findOneAndUpdate({'_token' : state.token}, userObject, {upsert:true}, done);
};
