...
DbWrapper.prototype.getItems = function(amount, first, last, done) {
    ...
    if (first && !last) {
        query = ItemModel.find({_sort : {$lt : first}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }

    //last is the sort time of the latest item seen
    if (!first && last) {
        query = ItemModel.find({_sort : {$gt : last}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }

    //outside of the seen window
    if (first && last) {
        query = ItemModel.find({_sort : {$gt : last, $lt : first}}, keepTheseKeys).sort({_sort : -1}).limit(amount > 99 ? 99 : amount);
    }
