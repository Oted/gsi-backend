...
Database.prototype.getQuery = function(requestQuery, done) {
    ...
    if (requestQuery.query) {
        hash = requestQuery.query;
    } else {
        var _unhash = Utils.generateUniqueQueryString(requestQuery);
        hash   = Utils.generateHash(_unhash);
    }

    return Models.model['query'].findOne({_hash : hash}, function(err, doc) {
        if (err) {
 ...
 */
Database.prototype.getRelatedFragments = function(fragment, done) {
    const amount = 15;
    ...
    return Models.searchItems(es_query, function(err, docs) {
        ...
        return done(null, Utils.getRelatedFragments(docs, fragment));
    });
};
...
Database.prototype.getFragments = function(amount, type, done) {
    ...
    switch (type) {
        ...
        case "random" :
            return Models.model['title-fragment'].find({}).limit(50).exec(function(err, docs) {
                if (err) {
                    return done(err);
                }

                return done(null, Utils.shuffle(docs).slice(0, amount || 50));
            });

        case "highlighted" :
            return Models.model['title-fragment'].find({highlighted : true}).limit(10).exec(done);
    }
};
...
Database.prototype.getItem = function(hash, done) {
    if (Cache.get('item.' + hash)) {
        console.log('Returning cache for getItem : ' + hash);
        return done(null, Cache.get('item.' + hash));
    }

    return Models.model['item'].findOne({_hash : hash}).exec(done);
};
...
Database.prototype.getItems = function(requestQuery, query, done) {
    ...
    if (requestQuery.first && !requestQuery.last) {
        es_query.query.bool.must_not.push({
            "range": {
                "_sort": {
                    "lte": parseInt(requestQuery.first)
                }
            }
        });
    }
    ...
    if (!requestQuery.first && requestQuery.last) {
        es_query.query.bool.must_not.push({
            "range": {
                "_sort": {
                    "gte": parseInt(requestQuery.last)
                }
            }
        });
    }
