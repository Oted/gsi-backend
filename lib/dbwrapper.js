...
//hot and new items are stored in memory and will have the structure [{_hash : {object....}}]
var hotItems    = [];
var hotSize     = 10;
 ...
 */
function DbWrapper(cache, url, done) {
    var that = this;
    Cache    = cache;
 ...
 */
DbWrapper.prototype.createFeedback = function(raw, done) {
    ...
    feedback.save(function(err, newDoc) {
        if (err) {
            return done(err);
        }
...
DbWrapper.prototype.vote = function(session, hash, ip, value, adjective, done) {
    ...
    return ItemModel.findOne({_hash  : hash}, function(err, docItem) {
        if (err) {
            return done(err);
        }
 ...
 */
DbWrapper.prototype.view = function(session, hash, ip, value, done) {
    if (!hash) {
        return done(new Error('Invalid hash'));
    }
    ...
    var token = session.token;
    ItemModel.findOneAndUpdate({_hash  : hash}, {$inc: {view_time : value}}, function(err, docItem) {
        if (err) {
            return done(err);
        }
 ...
 */
DbWrapper.prototype.addOrUpdateUserState = function(state, done) {
if (!state) {
...
try {
    ...
    return UserModel.findOneAndUpdate({'token' : state.token}, userObject, {upsert:true}, done);  
};
...
DbWrapper.prototype.getBestRatings = function(amount, types, adjectives, done) {
    ...
    if (!adjectives) {
        ...
        query = ItemModel.find({enabled : true, type : {$in : types}}, keepTheseKeys).sort({score : -1}).limit(amount);
