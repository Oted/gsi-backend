...
var index       = require('fs').readFileSync('gsi-react/public/index.html', 'utf8')
...
Router.prototype.renderTemplate = function(request, reply) {
    ...
    return Database.updateUser(request.yar.get('session'), {$inc : {visits : 1}} , function(err) {
        if (err) {
...
Router.prototype.renderThingsTemplate = function(request, reply) {
    ...
    return Database.getItem(thing, function(err, doc) {
        ...
        var output = Mustache.render(thingsTempl, {
            "header"  : doc.title
        });
