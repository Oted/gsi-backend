...
module.exports.transformQuery = function(query) {
    ...
    if (str.indexOf('$') > -1) {
        //to db
        if (query.title) {
            title = query.title.toString().match(/\/(.*)\//)[1];
        }
        ...
        //from db
        if (query.title) {
            title = new RegExp(query.title, 'gi');
        }
 ...
 */
module.exports.generateQueryHash = function(search, types, sources) {
    var s = sources.sort(function(a,b){return a > b}).join('').toLowerCase();
    var t = types.sort(function(a,b){return a > b}).join('').toLowerCase();

    var str = (search || '').toLowerCase() + s + t;

    return this.generateHash(str);
};

/**
 * Generates a query based on the search, types and sources
 */
module.exports.generateQuery = function(search, types, sources) {
    var q = {};

    q.enabled = true;

    q.title = new RegExp(search || '.*', "gi");
    q.type = {$in : types};
    q.source_type = {$in : sources};
